...
def main():
    ...
    parser.add_argument("number", type=int, help="Fibonacci serisinin üst sınır değeri")
    parser.add_argument(
        "--compare", action="store_true", help="Tüm algoritmaları karşılaştır"
    )
    parser.add_argument(
        "--formula", action="store_true", help="Kapalı formül kullanarak hesapla"
    )
    parser.add_argument(
        "--direct", action="store_true", help="Doğrudan yöntem kullanarak hesapla"
    )
    ...
    if args.formula:
        ...
        print(
            f"Fibonacci serisinin {args.number} sayısına kadar olan çift sayılarının toplamı: {result}"
        )
        return
